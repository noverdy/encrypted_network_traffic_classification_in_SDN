#!/usr/bin/python
import subprocess
import sys  # to handle the Ryu output
...
model_path = "./self_supervised.pth"
...
class Flow:
    ...
    def updateforward(self, packets, bytes):
        ...
        # if the flow did not receive any packets of bytes
...
def classify_feature(flow, model):
    t = PrettyTable(["Source IP", "Source Port", "Destination IP",
                    "Destination Port", "Predicted Class", "Inference Time (s)"])
    features = pd.DataFrame.from_dict(
        flow.feature_container).to_numpy().reshape((-1, 4, 45))
    # normalization
    ...
    t.add_row([flow.ethsrc, flow.inport, flow.ethdst, flow.outport,
              predicted_class, round(time.time()-start_time, 5)])
    print(t)
...
def save_feature(features, fid, traffic_type):
    ...
    fname = "./"+traffic_type+"/"+traffic_type + \
        "_"+str(fid)+"_"+str(FLOW_COUNTER)+".csv"
    FLOW_COUNTER += 1
    df.to_csv(fname, index=False)


def append_flow(flow, fid, traffic_type, model):
    flow.feature_container["delta_forward_packet"].append(
        flow.forward_delta_packets)
    flow.feature_container["delta_forward_byte"].append(
        flow.forward_delta_bytes)
    flow.feature_container["delta_reverse_packet"].append(
        flow.reverse_delta_packets)
    flow.feature_container["delta_reverse_byte"].append(
...
def run_ryu(p, traffic_type=None, model=None):
    ...
    while True:
        ...
        if out == '' and p.poll() != None:  # poll returns None if the process hasn't completed
            print("breaking")
            break
        # when Ryu 'simple_monitor_AK.py' script returns output
        if out != '' and out.startswith(b'data'):
            fields = out.split(b'\t')[1:]  # split the flow details

            fields = [f.decode(encoding='utf-8', errors='strict')
                      for f in fields]  # decode flow details
            # print(fields)
            # create unique ID using src_port, src_ip, dst_ip, dst_port
            unique_id = hash(
                ''.join([fields[2], fields[3], fields[4], fields[5]]))
            # print("Unique ID: ", unique_id)
            if unique_id in flows.keys():
                # update forward attributes with packet, and byte count
                flows[unique_id].updateforward(int(fields[6]), int(fields[7]))
                flow_status(traffic_type, model)
            else:
                # switch source and destination to generate same hash for src/dst and dst/src
                rev_unique_id = hash(
                    ''.join([fields[5], fields[4], fields[3], fields[2]]))
                # print("Reverse Unique ID: ", rev_unique_id)
                if rev_unique_id in flows.keys():
                    # update reverse attributes with packet, and byte count
                    flows[rev_unique_id].updatereverse(
                        int(fields[6]), int(fields[7]))
                    flow_status(traffic_type, model)
                else:
                    flows[unique_id] = Flow(int(fields[0]), fields[1], fields[2], fields[3], fields[4], fields[5], int(
                        fields[6]), int(fields[7]))  # create new flow object
                    flow_status(traffic_type, model)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 2:
        p = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE,
                             stderr=subprocess.STDOUT)  # start Ryu process
